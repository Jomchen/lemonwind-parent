* 森林转二叉树的公式需要补充
* 公式
   - `31 是个奇素数，JVM 会将 31 * i 优化为 (i << 5) - i`
      ...
      1. 如果一棵完全二叉树有 768 个节点，求叶子节点的个数
         ...
         完全二叉树的 n1 要么为0 要么为 1
          ...
          函数解析
             ...
             n 如果是奇数，叶n1 = 0，叶子节点数 n0 = (n+1)/2
             最终公式（适用于n为奇数或偶数时，推论如下）
                注意公式里的除法需要保留小数（非计算机性质的取商），然后按函数决定进位
...
* 红黑树
   ...
   红黑树添加后的调整（新添加的元素默认是红色）
      ...
      5-6. 添加-修复性质4 - LL/RR
         ...
         10, 20, 30, 36, 48, 74 先当做不存在（因为这几个都是之后单个分别作为新增案例的元素，如果此时当作存在实际上此图是不平衡的错误图。如果当作不存在，在未添加元素前是平衡的）
         52,60是新增红节点
                 *-----------38*<----55--------------*
                 |             |                     |
