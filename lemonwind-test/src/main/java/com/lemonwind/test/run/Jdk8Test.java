...
public class Jdk8Test {
    ...
    public static void test01() {
        ...
        Integer totalAge = Stream.iterate(0L, x -> x + 1).limit(10)
                .map(i -> new User(i, String.valueOf(i), (int)(i % 3), "北京" + i, new Date()))
                .collect(Collectors.reducing(0, User::getAge, Integer::sum));

        // 年龄分组，取得组中ID最大的对象
        Map<Integer, Optional<User>> maxIdMap = Stream.iterate(0L, x -> x + 1).limit(10)
                .map(i -> new User(i, String.valueOf(i), (int)(i % 3), "北京" + i, new Date()))
                .collect(groupingBy(User::getAge, maxBy(Comparator.comparing(User::getId))));

        // 年龄分组，取得组中ID最大的对象
        Map<Integer, User> maxIdMap2 = Stream.iterate(0L, x -> x + 1).limit(10)
                .map(i -> new User(i, String.valueOf(i), (int)(i % 3), "北京" + i, new Date()))
                .collect(
                        groupingBy(
                                User::getAge,
                                collectingAndThen(maxBy(Comparator.comparing(User::getId)), Optional::get)
                        )
                );

        // 年龄分组，结果映射为ID并收集为集合
        Map<Integer, List<Long>> sumIdMap = Stream.iterate(0L, x -> x + 1).limit(10)
                .map(i -> new User(i, String.valueOf(i), (int)(i % 3), "北京" + i, new Date()))
                .collect(
                        groupingBy(User::getAge, mapping(user -> user.getId(), toList()))
                );

        // 分区，分区组的 KEY 是布尔型
        Map<Boolean, List<User>> partitionMap = Stream.iterate(0L, x -> x + 1).limit(10)
                .map(i -> new User(i, String.valueOf(i), (int)(i % 3), "北京" + i, new Date()))
